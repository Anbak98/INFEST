...
public class PlayerInputActionHandler : MonoBehaviour
{
    ...
    // 애니메이션 회전을 위한 변수(카메라의 회전상태를 가져와야한다)
    public PlayerCameraHandler cameraHandler;
    public Vector2 LookDir { get; private set; } = Vector2.zero;  // 카메라가 보는 방향
    public float CameraX { get; private set; } = 0f;
    public float CameraY { get; private set; } = 0f;
    ...
    private void SetLookInput(InputAction.CallbackContext context)
    {
        ...
        // Camerahandler의 lookX, lookY를 가져온다
        CameraX = cameraHandler.lookX;
        CameraY = cameraHandler.lookY;
    }
    ...
    public NetworkInputData? GetNetworkInput()
    {
        var data = new NetworkInputData
        {
            ...
            lookDir = new Vector2(CameraX, CameraY),
            //LookX = CameraX,
            //LookY = CameraY,
